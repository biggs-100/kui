...
## NEW Capability: credential-storage
...
## MODIFIED Capability: provider-selection

### Requirement: REQ-SEL-3 — Fail-Fast API Key Validation

At provider creation time, the system MUST verify the API key is available through the resolution chain: environment variable > `.kui/credentials.json` > error. If missing at all layers, creation MUST fail immediately with an error naming the expected env var. (Previously: only checked environment variable.)
...
Provider selection MUST follow the layered chain: `--provider` flag > `provider:` field in profile.yaml > environment variable (e.g. `KUI_PROVIDER`) > default (`openai`). This requirement is unchanged.
