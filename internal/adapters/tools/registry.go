...
// write_file, edit_file, bash (REQ-TOOLS-4). File tools are confined to root;
// bashTimeout bounds every command (zero selects the default of 30s).
...
// When syncer is non-nil, read_file, write_file and edit_file tools send
// DidOpen/DidChange notifications to the LSP server via the syncer.
func DefaultWithSyncer(root string, bashTimeout time.Duration, syncer FileSyncer) []core.Tool {
	var rf, wf, ef core.Tool
	if syncer != nil {
		rf = NewReadFileWithSync(root, syncer)
		wf = NewWriteFileWithSync(root, syncer)
		ef = NewEditFileWithSync(root, syncer)
	} else {
		rf = NewReadFile(root)
